...
def create_mydb_database():
    ...
    with BPlusTree(filename=config.SCHEMA_TABLE_PATH, serializer=StrSerializer()) as f:
        ...
        f.insert(config.ADMIN_TABLE_NAME, user_table_schema.encode())

    # 2.创建user表，插入root用户
    with BPlusTree(filename=config.ADMIN_TABLE_PATH, serializer=StrSerializer()) as f:
        f.insert(key="root", value="root".encode())
